        ...
        {filtered.length > 0 ? (
          <div className="overflow-x-auto">
            <table className="w-full text-sm">
              <thead>
                <tr className="border-b border-sand-200 text-left text-xs font-semibold uppercase tracking-widest text-stone-500">
                  <th className="px-6 py-3.5 font-semibold">Documento</th>
                  <th className="px-6 py-3.5 font-semibold">Tipo</th>
                  <th className="hidden px-6 py-3.5 font-semibold md:table-cell">Responsable</th>
                  <th className="hidden px-6 py-3.5 font-semibold sm:table-cell">Tamaño</th>
                  <th className="px-6 py-3.5 font-semibold">Fecha</th>
                  <th className="px-6 py-3.5 text-right font-semibold">Acción</th>
                </tr>
              </thead>
              <tbody className="divide-y divide-sand-200">
                {filtered.map((doc) => (
                  <tr key={doc.id} className="group transition-colors hover:bg-sand-50">
                    ...
                    <td className="hidden px-6 py-3.5 text-stone-600 md:table-cell">
                      {doc.owner}
                    </td>
                    <td className="hidden px-6 py-3.5 text-stone-600 sm:table-cell">
                      {formatFileSize(doc.sizeKb)}
                    </td>
                    <td className="px-6 py-3.5 text-stone-600">{formatDate(doc.uploadedAt)}</td>
          ...
          <div className="flex flex-col items-center justify-center py-20 text-center">
            <span className="flex h-12 w-12 items-center justify-center rounded-full bg-sand-100 text-stone-400">
              <FileX size={22} />
            </span>
            <p className="mt-4 text-sm font-medium text-stone-900">Sin resultados</p>
